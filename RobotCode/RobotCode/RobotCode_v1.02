...
public:
	...
	int autoLoopCounter;
	Robot() :
			myRobot(0, 1), table(NULL), stick(0), ahrs(NULL), lw(NULL), autoLoopCounter(
					0) {
...
private:
	...
	void TeleopPeriodic() override {
		...
		if (ThisButState[FastModeBut])
			FastMode = true;  //切换为快速模式
		if (ThisButState[SlowModeBut])
			FastMode = false;  //切换为微调模式
		...
		if (ThisButState[FastClimbBut])
			CimbingMotor.Set(FastClimbSpeed);  //快速爬绳
		if (ThisButState[SlowClimbBut])
			CimbingMotor.Set(SlowClimbSpeed);  //慢速爬绳
		*/
		//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
///降铲子
	...
	bool MoveToPos(double XPos, double YPos, double FinalFacingAngle,double MovingSpeed) {
		...
		if (InRange(XPos, distol, ahrs->GetDisplacementX())
				&& (InRange(YPos, distol, ahrs->GetDisplacementY()))) {
			if (InRange(ahrs->GetYaw(), angletol, FinalFacingAngle)) {
				return true;
			} else {
				myRobot.Drive(0,
						-(FinalFacingAngle - ahrs->GetYaw()) * P_angle);
			}
		} else {
			...
			if (InRange(ahrs->GetYaw(), angleact, TargetAngle)) {
				myRobot.Drive(MovingSpeed,
						-(TargetAngle - ahrs->GetYaw()) * P_angle);
			} else {
